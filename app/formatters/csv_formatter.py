...
class CSVFormatter:
    ...
    def format(self, data: Dict[str, Any], tables: list = None) -> str:
        ...
        if data.get('registration'):
            writer.writerow(['Company Registration', 
        ...
        if data.get('total'):
            writer.writerow(['Total Amount', 
                           self.data_formatter.format_field('currency', data['total'])])
        if data.get('tax'):
            writer.writerow(['Tax', 
                           self.data_formatter.format_field('currency', data['tax'])])
        
        # Payment Information
        if data.get('bank_account'):
            writer.writerow(['Bank Account', 
